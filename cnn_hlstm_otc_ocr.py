...
class LSTMOCR(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('HLSTM'):
            lstm_fw = hlstm(FLAGS.num_hidden, FLAGS.rnn_layers)
            lstm_bw = hlstm(FLAGS.num_hidden, FLAGS.rnn_layers)
            output, state = tf.nn.bidirectional_dynamic_rnn(lstm_fw, lstm_bw, x , scope='bi_hlstm' , dtype=tf.float32)
            x = tf.concat(output, axis=2)
            print('hlstm' + ':  ', x.get_shape())
